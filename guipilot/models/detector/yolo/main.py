...
def create_coco_json(annotation_list, label2id, output_json_path, coco_format):
    ...
    for annotation in annotation_list:
        ...
        if image_name not in image_dict:
            ...
            # Add image info
            coco_format["images"].append({"id": image_id, "width": width, "height": height, "file_name": image_name})
            image_id += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--json_train", type=str, default="./datasets/80app_train_groundtruth.json", help="save the training data as json"
    )
    parser.add_argument(
        "--json_test", type=str, default="./datasets/80app_test_groundtruth.json", help="save the test data as json"
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    if args.prepare_data:
        ...
        create_coco_json(
            annotation_list=train_annotation_list, label2id=label2id, output_json_path=args.json_train, coco_format=coco_format
        )
        ...
        create_coco_json(
            annotation_list=test_annotation_list, label2id=label2id, output_json_path=args.json_test, coco_format=coco_format
        )
