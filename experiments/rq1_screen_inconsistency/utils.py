...
def visualize_inconsistencies(
    s1: Screen, s2: Screen, pairs: list[tuple], inconsistencies: list[tuple], path: str, filename: str
):
...
def load_screen(image_path: str) -> Screen:
    ...
    widgets = {
        i: Widget(type=widget_type, bbox=bbox) for i, (widget_type, bbox) in enumerate(zip(sorted_labels, sorted_bboxes))
    }
