    ...
    for process_path in process_paths[:5]:  # 只检查前5个
        ...
        if has_mockup and has_implementation and has_json:
            ...
            try:
                ...
                print(f"    ✗ process.json 解析失败: {e}")
        else:
            missing = []
            if not has_mockup: missing.append("mockup/")
            if not has_implementation: missing.append("implementation/")
            if not has_json: missing.append("implementation/process.json")
